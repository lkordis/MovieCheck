...
class MovieGridOld extends Component {
    constructor(props) {
        super(props);
        ...
        addEventListener('search', (e) => {
            if (e.detail === '') {
                ...
                this.limiter.removeTokens(1, () => {
                    ...
                    sessionStorage.setItem('query', e.detail)
                    this.props.search(e.detail).then((results) => {
    ...
    handleScroll(event) {
        ...
        if (this.props.props instanceof DiscoverApiData && !this.state.searching) {
            ...
            if ($(document).height() - win.height() === win.scrollTop()) {
                ...
                this.props.search(sessionStorage.getItem('query')).then(results => {
